...
def show_summary(actual, prediction, probabilities):
    """Function to report Accuracy, Precision, Recall and ROC AUC of a Classification model.
    ...
    print('Accuracy : %.4f [TP / N] Proportion of predicted labels that match the true labels.' % accuracy_score(actual, prediction))
    print('Precision: %.4f [TP / (TP + FP)] Not to label a negative sample as positive.' % precision_score(actual, prediction))
    print('Recall   : %.4f [TP / (TP + FN)] Find all the positive samples.' % recall_score(actual, prediction))
    print('AUC  : %.4f' % auc_score(actual, probabilities[:, 1]))
    print('F1 Score : %.4f' % f1_score(actual, prediction))